...
def flattenChildren(childList):
	...
	return sorted(list(timeToTuple.values()), None, lambda tup: tup[0])
...
def mergeRenderedChildren(childFrac, children):
	...
	for i in range(len(children)):
		...
		merge += list(map(timeShift, children[i]))
...
class PydalNode:
	def __init__(self, children):
		...
		self.frac = sum([c.frac for c in children])
	...
	def render(self, frac=None):
		...
		childFracs = [node.frac*scale for node in self.children]
		...
		for i in range(len(childFracs)-1):
			...
			merge += list(map(timeShift, self.children[i].render(childFracs[i+1])))
