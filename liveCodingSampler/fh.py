...
class FH:
	...
	def __init__(self):
		...
		self.scales = [[0, 2, 3, 5, 7, 8, 10] for i in range(n-1)] + [list(range(12))]
	...
	def saveNewLaunchpadLoop(self, addr, tags, stuff, source):
		...
		self.scales[chanInd] = list(map(int, stuff[3].split(",")))
	...
	def stringToHitList(loopString):
		...
		recBuf = list(map(splitHit, lineSplit[1].split("-")))
		return recBuf, lineSplit[0], lineSplit[2]

	@staticmethod
	def hitListToString(hitList, button, startBeat, playing=0):
		return str(button) + " " + "-".join([",".join(map(str, h)) for h in hitList]) + " " + str(startBeat) + " " + str(playing)
	...
	def sendScene(self, loops, loopInfo, roots, scales, faderBanks, currentFaderVals):
		...
		banksToString = lambda a: "-".join([".".join([",".join(map(str,slot)) for slot in bank]) for bank in a])
		currentFadersToString = lambda bank: ".".join([",".join(map(str,slot)) for slot in bank])
		msg = OSC.OSCMessage()
	...
	def saveScene(self, ind):
		c = copy.deepcopy
		self.scenes[ind] = list(map(c, self.getScene()))
		msg = OSC.OSCMessage()
	...
	def recieveCurrentFaderVals(self, addr, tags, stuff, source):
		...
		self.currentFaderVals = [list(map(int, s.split(","))) for s in stuff[0].split(".")]
		currentFadersToString = lambda bank: ".".join([",".join(map(str,slot)) for slot in bank])
	...
	def playScene(self, ind):
		c = copy.deepcopy
		self.sceneStack.append(list(map(c, self.getScene())))
	...
	def saveFaderSetting(self, addr, tags, stuff, source):
		...
		self.faderBanks[stuff[0]][stuff[1]] = list(map(int, stuff[2].split(",")))
	...
	def rootScale(self, chan=0, root=0, scale='minor'):
		...
		msg.append(root)
		keyval = scale
		if scale in list(phrase.modes.keys()):
			keyval = ",".join(map(str, phrase.modes[scale]))
		else:
			keyval = scale.split(',')
			keyval = [int(a.strip()) for a in keyval]
			if len(keyval) == 0:
...
def flattenByBeat(byBeat):
	...
	for i in range(len(byBeat)):
		flattened += [[note[0]+i] + note[1:] for note in byBeat[i]]
	return flattened
...
def warp(hitList, warpPoint, constant=.2, exponent=1):
	...
	warpList = [calcWarp(note[0], warpPoint, constant, exponent) for note in noteList]
...
def spatialize(hitList, root, noteSets):
	newNoteSets = [[(root+degree)%12 for degree in noteSet] for noteSet in noteSets]
