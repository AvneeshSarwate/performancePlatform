...
fl = Math.floor;
function calculateRegionalDif(){
	matrixHistory[historyInd].frommatrix(thresholdMatrix);
	for(var i = 0; i < xDim; i++){
		for(var j = 0; j < yDim; j++) {
			var regionInd = fl(i / fl(xDim/xSplit)) * xSplit + fl(j / fl(yDim/ySplit));
			var pixelDiff = Math.abs(matrixHistory[mod(historyInd-1)].getpixel(i, j) - thresholdMatrix.getpixel(i,j));
			historyRegionDiffs[historyInd][regionInd] += pixelDiff / 255;
		}
	}
}
